...
- Pan (drag). Zoom with the on-canvas + / − / fit buttons, mouse wheel (or trackpad pinch) over the canvas, or `+` / `-` / `0` (fit). Fit scales so the graph width fills the canvas; height may overflow and is panned. Zoom-in cannot exceed that width-fit scale.
