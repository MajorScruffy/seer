...
- Nested boxes, not arrows. When one entry is selected it is an outer box. Direct callees are **inside** it, left to right in `seq` order. If `main` calls `a()` then `b()` then `c()`, the `main` box contains three inner boxes. Fill color interpolates from a first-level color to a last-level color by nest depth (outer = first, deepest visible box = last).
- The `all` view groups instead of stacking entries flat: one outer box per **crate role**, inside it one box per **file**, inside each file the boxes for that file's functions (each still nesting its callees as above). Crate role is derived from the node `file` path: `src/main.rs` → `bin/seer`, `src/bin/<name>.rs` → `bin/<name>`, other `src/…` → `lib`, `tests/fixtures/…` → `fixtures`, other `tests/…` → `tests`, anything else → its first path segment (or `other` when there is none). Group and file boxes are structural: they carry no source, they start collapsed, and each shows a triangle that expands it to full size (showing everything inside). Collapse is per group / per file.
- Boxes with callees start **collapsed**. Every box with callees shows a triangle. Clicking it expands that box to full size (showing the functions it calls); clicking again collapses it back to header size. Collapse state is per nest path (the same function under two callers is two boxes) and resets when the entry filter changes.
